...
//        String SQL = "INSERT INTO (intern_id,mis_name,rate,date,hours,description) Schedule_Mis_Exten value (?,?,?,(select mis_id from mission where mis_name =?),?,?)";
//        try (
//                PreparedStatement ps = con.prepareStatement(SQL)) {
...
//        }
//    }
    public void Addschedule(int internId,String mis_name,float rate,Timestamp date,int hours,String description) {
        String SQL = "INSERT INTO (intern_id,mis_name,rate,date,hours,description) Schedule_Mis_Exten value (?,?,?,?,?,?)";
        try (
                PreparedStatement ps = con.prepareStatement(SQL)) {
            ps.setInt(1, internId);
            ps.setString(2,mis_name);
            ps.setFloat(3, rate);
