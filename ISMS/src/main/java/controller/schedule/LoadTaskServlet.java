...
public class LoadTaskServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        List<ScheduleMission> scheduleMissionsList = scheduleMissionDAO.getAllScheduleMissions(internId);

        List<Integer> checkedTaskIds = new ArrayList<>();
        for (Task task : taskList) {  // Giả sử bạn có một danh sách taskList
            if (task.isIsDone() == true) {
                checkedTaskIds.add(task.getTaskId());
            }
        }
        
        session.setAttribute("scheduleId", scheduleId); 
        request.setAttribute("checkedTaskIds", checkedTaskIds);
